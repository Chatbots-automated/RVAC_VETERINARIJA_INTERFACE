-- =====================================================================
-- Update Withdrawal Report to Use Veterinarians Table
-- =====================================================================
-- Migration: 20260429000017
-- Created: 2026-04-29
-- =====================================================================
...
-- Branch 1: Medications/Vaccines from usage_items
SELECT 
    t.farm_id,
    f.name AS farm_name,
    f.code AS farm_code,
    ...
    COALESCE(vet.name, t.vet_name) AS vet_name,  -- ✅ Use veterinarians table
    COALESCE(vet.name, t.vet_name) AS veterinarian,  -- ✅ ALIAS for frontend
    t.notes,
    t.clinical_diagnosis,
    t.clinical_diagnosis AS disease_name,
    t.disease_id,
...
SELECT
    ...
    p.withdrawal_days_milk AS product_base_withdrawal_milk,
    ss.dosage AS dose,
    CONCAT(ss.dosage::text, ' ', ss.dosage_unit::text) AS quantities_used,
    ...
    'Sinchronizacijos protokolas' AS veterinarian,
    sp.name AS notes,
...
'Withdrawal report for all treatments (medicines and vaccines). Uses veterinarians table when available. Applies eco farm 2x multiplier dynamically.';
...
'Alias for vw_withdrawal_report - shows withdrawal data for all farms.';
