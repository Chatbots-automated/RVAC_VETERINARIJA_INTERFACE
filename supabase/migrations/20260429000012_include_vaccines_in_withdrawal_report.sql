-- =====================================================================
-- Fix: Include VACCINES in IŠLAUKŲ ATASKAITA (not just medicines)
-- =====================================================================
-- Migration: 20260429000012
-- Created: 2026-04-29
--
-- PROBLEM:
-- vw_withdrawal_report was filtering WHERE p.category = 'medicines'
-- But treatments include BOTH medicines AND vaccines
-- This caused:
-- - GYDOMŲ GYVŪNŲ REGISTRACIJOS ŽURNALAS: 60 rows (all products)
-- - IŠLAUKŲ ATASKAITA: 0 rows (only medicines = none found)
--
-- SOLUTION:
-- Change filter to: WHERE p.category IN ('medicines', 'vaccines')
-- Now both reports will have the same row count
-- =====================================================================
...
WHERE p.category IN ('medicines', 'vakcina')  -- ✅ CHANGED: Include both medicines AND vaccines
  -- NO FILTER by withdrawal dates! Show ALL medicine/vaccine treatments!
...
WHERE ss.completed = true 
  ...
  AND p.category IN ('medicines', 'vakcina');  -- ✅ CHANGED: Include both medicines AND vaccines

COMMENT ON VIEW public.vw_withdrawal_report IS 
'Withdrawal report for all treatments (medicines and vaccines). Applies eco farm 2x multiplier dynamically in the view. Shows same row count as GYDOMŲ GYVŪNŲ REGISTRACIJOS ŽURNALAS.';
