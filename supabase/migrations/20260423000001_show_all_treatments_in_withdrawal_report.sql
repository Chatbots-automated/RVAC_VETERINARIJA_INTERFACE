-- =====================================================================
-- Show ALL Treatments in Withdrawal Report
-- =====================================================================
-- Created: 2026-04-23
-- Description:
--   Updates vw_withdrawal_report and vw_withdrawal_journal_all_farms
--   to show ALL treatments (not just those with withdrawal periods).
--   Treatments without withdrawal will show "Nėra" in the frontend.
--   This makes the withdrawal report have the same rows as the 
--   Treated Animals Register (GYDOMŲ GYVŪNŲ REGISTRACIJOS ŽURNALAS).
-- =====================================================================
...
-- Now showing ALL treatments
ORDER BY 
    f.name ASC,
    t.reg_date DESC;

COMMENT ON VIEW public.vw_withdrawal_report IS 'All treatments for withdrawal report - includes treatments without withdrawal periods (will show Nėra in frontend). Includes eco-farm logic: 0 days becomes 2, others are multiplied by 2';

-- Update all-farms withdrawal journal view with same logic
...
LEFT JOIN public.diseases d ON t.disease_id = d.id
-- REMOVED: WHERE clause that filtered only treatments with withdrawal
-- Now showing ALL treatments across all farms
ORDER BY 
    f.name ASC,
    t.reg_date DESC;

COMMENT ON VIEW public.vw_withdrawal_journal_all_farms IS 'Farm-wide withdrawal journal showing all treatments across all farms (even without withdrawal periods). Includes eco-farm logic: 0 days becomes 2, others are multiplied by 2';
