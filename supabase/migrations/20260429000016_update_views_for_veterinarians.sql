-- =====================================================================
-- Update Views to Use Veterinarians Table
-- =====================================================================
-- Migration: 20260429000016
-- Created: 2026-04-29
--
-- OVERVIEW:
-- Update vw_treated_animals_detailed and vw_withdrawal_report to show
-- veterinarian name from veterinarians table when available, falling back
-- to vet_name when veterinarian_id is NULL
-- =====================================================================
...
SELECT
    ...
    t.outcome_date,
    COALESCE(vet.name, t.vet_name, 'Nenurodyta') AS veterinarian,  -- ✅ Use veterinarians table
    t.notes,
...
LEFT JOIN public.veterinarians vet ON t.veterinarian_id = vet.id  -- ✅ JOIN veterinarians
WHERE NOT EXISTS (
...
GROUP BY 
    ...
    t.outcome, t.outcome_date, vet.name, t.vet_name, t.notes, t.created_by_user_id, t.created_at,
    ui.administered_date, f.is_eco_farm, p.withdrawal_days_meat, p.withdrawal_days_milk

UNION ALL

-- Branch 3: Synchronization medicines
SELECT
    ui.farm_id,
...
COMMENT ON VIEW public.vw_treated_animals_detailed IS 'Treated animals with medications. Uses veterinarians table when available. Eco farms: 2x withdrawal multiplier applied dynamically.';
