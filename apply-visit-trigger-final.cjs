...
(async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('=== PROTECTION NOW ACTIVE ===');
      console.log('When marking visits as "Baigtas", the system will automatically:');
      console.log('  ✓ Create usage_items from planned medications');
      console.log('  ✓ Calculate withdrawal periods');
      console.log('  ✓ Deduct stock from batches');
      console.log('\nThe issue with cow LT000008370321 will NOT happen again!');
    } else {
      throw new Error('Failed to apply trigger');
    }
  } catch (error) {
    console.log('⚠️  Cannot apply trigger automatically\n');
    console.log('MANUAL APPLICATION REQUIRED:');
    console.log('1. Go to: https://supabase.com/dashboard/project/olxnahsxvyiadknybagt/sql');
    console.log('2. Copy and paste the SQL from: auto_process_visit_medications_trigger.sql');
    console.log('3. Click "Run"\n');
    console.log('The cow LT000008370321 is ALREADY FIXED.');
    console.log('This trigger will prevent the issue from happening to other cows.');
  }
})();
